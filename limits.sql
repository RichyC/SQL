...
PRAGMA EXCEPTION_INIT(too_many_items, -20001);
bag_too_heavy EXCEPTION;
PRAGMA EXCEPTION_INIT(bag_too_heavy, -20002);
item_too_heavy EXCEPTION;
PRAGMA EXCEPTION_INIT(item_too_heavy, -20003);
BEGIN
...
ROLLBACK;
WHEN too_many_items THEN
DBMS_OUTPUT.PUT_LINE('You do not have room for this item.');
ROLLBACK;
WHEN bag_too_heavy THEN
DBMS_OUTPUT.PUT_LINE('Your bag is too heavy to lift.');
ROLLBACK;
WHEN item_too_heavy THEN 
DBMS_OUTPUT.PUT_LINE('The item is too heavy for you to lift');
ROLLBACK;
WHEN OTHERS THEN
...
CREATE OR REPLACE TRIGGER count_check
BEFORE INSERT ON Explorer_Treasure
FOR EACH ROW
DECLARE
max_items Explorer.Max_Item_Count%TYPE;
current_item_count Explorer.Max_Item_Count%TYPE;
BEGIN
SELECT Max_Item_Count
INTO max_items
FROM Explorer
WHERE Exp_ID = :NEW.Exp_ID;
SELECT sumTable.total_items
INTO current_item_count
FROM (SELECT Count(*) AS total_items FROM Explorer_Treasure
WHERE Exp_ID = :NEW.Exp_ID) sumTable;
IF (current_item_count + 1 > max_items) THEN
RAISE_APPLICATION_ERROR(-20001, 'Too many items in bag');
END IF;
END;
/

CREATE OR REPLACE TRIGGER weight_check
BEFORE INSERT ON Explorer_Treasure
FOR EACH ROW
DECLARE
bag_weight Explorer.Max_Bag_Weight%TYPE;
item_weight Treasure.Weight%TYPE;
bag_weightMAX Explorer.Max_Bag_Weight%TYPE;
item_weightMAX Treasure.Weight%TYPE;
BEGIN
SELECT sumTable.total_weight
INTO bag_weight
FROM (SELECT SUM(Treasure.Weight) AS total_weight
FROM Treasure INNER JOIN Explorer_Treasure
ON Explorer_Treasure.Tres_ID = Treasure.Tres_ID
WHERE Exp_ID = :NEW.Exp_ID) sumTable;
SELECT Weight
INTO item_weight
FROM Treasure
WHERE Tres_ID = :NEW.Tres_ID;
SELECT Max_Bag_Weight
INTO bag_weightMAX
FROM Explorer
WHERE Exp_ID = :NEW.Exp_ID;
SELECT Max_Item_Weight
INTO item_weightMAX
FROM Explorer
WHERE Exp_ID = :NEW.Exp_ID;
IF (bag_weight + item_weight > bag_weightMAX) THEN
RAISE_APPLICATION_ERROR(-20002, 'Bag is too heavy');
END IF;
IF (item_weight > item_weightMAX) THEN 
RAISE_APPLICATION_ERROR(-20003, 'Item is too heavy');
END IF;
END;
/
